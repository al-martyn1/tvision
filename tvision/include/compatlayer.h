...
/* But it isn't portable:
   ...
   Currently we assume that's safe, but any platform where it isn't can change
   it.
*/
...
/* GNU C is supported for various OSs: */
    ...
    #define IfStreamGetLine(istream, buffer, size) istream.getline(buffer, size)
    ...
    /* ISO C++ 1998 standard changed a lot of details in the standard C++
       ...
       stuff in versions like BC++ 5.5. So that's a real mess. */
            ...
            // and C++ standard says inclusion of this file has no effect, to
            // detect libc++
            ...
            #define CLY_NewFBFromFD(buf, f)                 \
                buf = new CLY_int_filebuf(fdopen(f, "rb+"), \
            ...
            #define CLY_filebuf std::filebuf
                ...
                #define CLY_NewFBFromFD(buf, f) \
                    buf = new CLY_int_filebuf(  \
                        fdopen(f, "rb+"), ios::in | ios::out | ios::binary)
            #else
                #undef CLY_DefineSpecialFileBuf
                #define CLY_DefineSpecialFileBuf 1
                #define CLY_NewFBFromFD(buf, f)                       \
                    buf = new CLY_int_filebuf(f, ios::in | ios::out | \
                                                     ios::binary)
            #endif
            #undef FSTREAM_HEADER
            #define FSTREAM_HEADER <fstream>
        ...
        #define CreateStrStream(os, buf, size) \
            std::string        buf;            \
    ...
    #define False   false
        ...
        /* SET: Not sure why Vadim wanted it. Perhaps to make the port more
           usable for users accustomed to back slashes, but then the library
...
CLY_CFunc void _fixpath(const char *in, char *out);
        ...
        #endif
        #ifdef Uses_utime
            /* I can't beleive they forgot the header when the function is
             * there! */
...
CLY_CFunc int mkstemp(char *_template);
        ...
        #endif
        #if defined(Uses_alloca) && (__MINGW32_MAJOR_VERSION >= 2) && \
            !defined(alloca)
...
CLY_CFunc int strcasencmp(const char *s1, const char *s2, size_t n);
    ...
    /* Generic UNIX system */
        ...
        !defined(TVOSf_Solaris) && !defined(TVOSf_OpenBSD) && \
        !defined(TVOSf_NetBSD)
        #ifdef Uses_getopt
            #undef Include_cl_getopt
            #define Include_cl_getopt 1
        #endif
...
enum Boolean
{
    ...
    True
        ...
        #undef AllocLocalUShort
        #define AllocLocalUShort(s, l) \
            ushort *s = (ushort *)alloca(sizeof(ushort) * (l))
        #undef Uses_alloca
...
CLY_CFunc int mkstemp(char *_template);
    ...
    #define IfStreamGetLine(istream, buffer, size) \
        ...
        #define CLY_NewFBFromFD(buf, f) \
            ...
            buf->open(f)
        #undef CLY_destroy
        // Note: that 0777 fails to create the file and 0666 creates a hidden
        // file.
        #define CLY_FBOpenProtDef 0
    #else
...
/* Derived from BC++ section */
    ...
    /* Watcom generates the following error for validate.h:
       include\tv\validate.h(127): Error! E346: col(33) protected base class
       accessed to convert cast expression So we just use public there.
    */
    #undef CLY_BROKEN_WATCOM_SCOPE
...
/* MSVC will be supported if volunteers tests it or Microsoft decides to
   give it for free ;-). After all Borland released BC++ 5.5. */
    ...
    #if !defined(__MSC_VER) // let M$ compiler be version 1.00, if nothing else
                            // has been defined.
...
/*
 ...
 It looks like you must add 6 to get the compiler version. I guess they created
MSVC 1.0 when compiler version 7.0 was released. Compiler version   MSVC version
      10.00             4.0
...
*/
#ifdef TVComp_MSC
    // Note: snprintf and vsnprintf seems to be available in some versions but
    // not in others. But looks like _snprintf and _vsnprintf is available in
    // 1000 and 1300 versions. So currently I assume the _* versions are
    // available for all versions.
    //
    // Note 2: at least MSVC 1200 (6.0) have a wrong implemetation of vsnprintf,
    // it have an assert checking that the buffer is != NULL and/or size != 0.
    // This is wrong. According to the IEEE 1003.1 (POSIX 1003.1) standard
    // (Issue 6 2004):
    // ... "If /n/ is zero, nothing shall be written and /s/ may be a null
    // pointer." ... And when the result value is explained says: "Upon
    // successful completion, the /snprintf/() function shall return the number
    // of bytes that would be written to /s/ had /n/ been sufficiently large
    // excluding the terminating null byte."
    //
    // As I don't know which version fixed this bug I use the replacement that
    // really works.
    //
    #undef CLY_Have_snprintf
    #define CLY_UseCrLf          1
    ...
    */
    ...
    /*
      ...
      ISO C++ conformant name: _access. See online help for details.' This is a
      nonsense, POSIX is an standard and changing the names of the functions
      makes the code incompatible with POSIX compliant compilers. I don't think
      that's a solution to make the compiler ISO C++ compliant. It could be a
...
CLY_CFunc int mkstemp(char *_template);
    ...
    #undef CLY_DummyTStreamRW
    #define CLY_DummyTStreamRW(cla)                    \
        void  write(opstream &os) { cla::write(os); }; \
    ...
    // v1310 is MSVC 7.10 (aka .NET 2003 Architect), ISO C++ 1998 compliant
        ...
        #define GetStrStream(os, buf) os.str().c_str()
            ...
            #define Include_sstream 1
        #endif
        // I'm not sure about the version, but .NET 2003 seems to only have the
        // headers without .h
        #undef FSTREAM_HEADER
        #define FSTREAM_HEADER <fstream>
        #undef IOMANIP_HEADER
...
// #endif
    ...
    #define IOS_BIN CLY_IOSBin
        ...
        #define Include_stdio 1
        #ifdef TVComp_MSC
            // Note: Currently unused because the implementation is wrong. See
            // notes in the MSVC section.
...
// Simple Streams Compatibility hack *EXPERIMENTAL*
    ...
    #define CLY_FBOpenProtDef 0666
    #define CLY_NewFBFromFD(buf, f) \
        buf = new SSC_filebuf(f, SSC_ios::in | SSC_ios::out | SSC_ios::binary)
    #define CLY_PubSetBuf(a, b) setbuf(a, b)
    #define CLY_HaveFBAttach    1
    #define CLY_FBOpen(a, b, c) open(a, b, c)
...
typedef long ssize_t;
    ...
    #include <io.h>
        ...
        /* Needed for MSVC */
        /* Note we must avoid macros here or we risk to collide with TV
           read/write C++ members. */
        #ifndef CLY_close_inline_defined
            #define CLY_close_inline_defined 1
__inline int close(int FILEDES) { return _close(FILEDES); }
        #endif
        #ifndef CLY_dup_inline_defined
            #define CLY_dup_inline_defined 1
__inline int dup(int OLD) { return _dup(OLD); }
        #endif
        #ifndef CLY_dup2_inline_defined
            #define CLY_dup2_inline_defined 1
__inline int dup2(int OLD, int NEW) { return _dup2(OLD, NEW); }
        #endif
...
CLY_EXPORT CLY_CFunc void CLY_GetFileAttributes(CLY_mode_t  *mode,
                                                struct stat *statVal,
                                                const char  *fileName);
/* The reverse. The file must be closed! */
CLY_EXPORT CLY_CFunc int CLY_SetFileAttributes(CLY_mode_t *newmode,
                                               const char *fileName);
...
CLY_EXPORT CLY_CFunc int CLY_vsnprintf(char *string, size_t length,
                                       const char *format, va_list args);
CLY_EXPORT CLY_CFunc int CLY_snprintf(char *string, size_t length,
                                      const char *format, ...);
#endif
...
CLY_EXPORT int CLY_IfStreamGetLine(CLY_std(ifstream) & is, char *buffer,
                                   unsigned len);
#endif
...
CLY_CFunc CLY_EXPORT ssize_t CLY_getline(char **lineptr, size_t *n,
                                         FILE *stream);
